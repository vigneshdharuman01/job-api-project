  ...
  return (
    <div className={`p-5 ${loader ? "loader" : null}`}>
      {loader ? <Spinner animation="border" variant="primary" /> : 
         ...
         <tbody>
           {apiData?.map((item,index)=>{
             return (
               ...
               <td>{item.language}</td>
               <td>{item.state}</td>
               <td><button className="btn btn-success me-2" onClick={()=> editData(item) }>Edit</button>
       ...
       <i className="addData" onClick={()=> createData()}>  <MdAddCircle /> </i>
     
       <Model modelShow={show} modelClose={handleClose} modelData={temp} setModelData={setTemp} setRefresh={setRefresh} refresh={refresh}/>
       </>
      }
